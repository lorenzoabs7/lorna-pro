...
import { c as createTranslator } from '../../chunks/translator_BjG9bXh9.mjs';
export { renderers } from '../../renderers.mjs';

const resend = process.env.RESEND_API_KEY ? new Resend(process.env.RESEND_API_KEY) : null;
const t = createTranslator("en");
const dbPath = typeof process.env.VERCEL !== "undefined" ? require$$0.join("/tmp", "contact_submissions.db") : require$$0.join(process.cwd(), "contact_submissions.db");
let db = null;
function getDb() {
  if (db) return db;
  try {
...
function errorResponse(errorKey, status) {
  return new Response(
    JSON.stringify({
      errorKey,
      error: t.t(errorKey)
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <div style="background: #e9ecef; padding: 15px; border-radius: 8px; margin: 20px 0; font-size: 14px;">
                  ...
                  <p><strong>${t.t("api.contact.email.submitted")}</strong> ${(/* @__PURE__ */ new Date()).toLocaleString()}</p>
